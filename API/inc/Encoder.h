...
/*
 * Core 编码器 ID：
 * F103: 用 TIM 编号（TIM2=1, TIM4=3）区分
 * F407: 用 TIM 编号（TIM1=0, TIM4=3）区分
 * G3507: 用编码器序号（0/1）区分
 */
#if (ENROLL_MCU_TARGET == ENROLL_MCU_F103)
...
/* 编码器配置表项：逻辑 id 绑定到物理硬件实例 + 两路信号引脚。 */
typedef struct
{
	API_Encoder_Id_t id;
	uint8_t          coreId;
